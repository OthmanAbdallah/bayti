    ...
    await firestore.collection(AccountType).doc(uid).collection("unrated_Jobs").get().then((snapshot) async {
      ...
      if(templist.isEmpty)
        ...
        for (jobId in templist){
         ...
         showRateModal(context, jobId, jobdocumentSnapshot["user_uid"], jobdocumentSnapshot["worker_uid"], jobdocumentSnapshot["user_name"], jobdocumentSnapshot["worker_name"], jobdocumentSnapshot["service"],jobdocumentSnapshot["area"]);
